...
describe("Titan type checker", function()
    ...
    it("coerces to integer", function()
        ...
        assert.truthy(ok)
        assert.same("Exp_Cast", ast[1].block.stats[2].exps[1]._tag)
        assert.same("Integer", ast[1].block.stats[2].exps[1].target._tag)
    end)
    ...
    it("coerces to float", function()
        ...
        assert.truthy(ok)
        assert.same("Exp_Cast", ast[1].block.stats[2].exps[1]._tag)
        assert.same("Float", ast[1].block.stats[2].exps[1].target._tag)
    end)
